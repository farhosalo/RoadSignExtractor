...
class RoadSignExtractor:
    __GDinoUrl = "https://github.com/IDEA-Research/GroundingDINO/releases/download/v0.1.0-alpha/groundingdino_swint_ogc.pth"
    ...
    def __DownloadModel(self):
        fileName = self.__GDinoUrl.rsplit("/", 1)[-1]
        completeFileName = os.path.join(self.__WeightDir, fileName)

        if not os.path.exists(completeFileName):
            request.urlretrieve(self.__GDinoUrl, completeFileName)

    def __LoadModel(self):
        if self.__Model == None:
            configPath = "config/GroundingDINO_SwinT_OGC.py"
            checkPoint = os.path.join(
                self.__WeightDir, self.__GDinoUrl.rsplit("/", 1)[-1]
            )
            self.__Model = Model(
